<?php
        $slctBangunan = $_POST['slctBangunan'];
        $slctAdmin = $_POST['slctAdmin'];

        $db = new PDO('pgsql:host=localhost;port=5432;dbname=sigwebngrowo;','postgres','dimas12345');
        $sql = $db->prepare("SELECT id_bang,nomen_klat,nama_bangu,jenis_bang,petugas,lokasi_rua,tipe_kerus,kondisi_fi,kondisi_fu,jenis_pena,desa,kecamatan,kabupaten,nama_sunga,nama_das FROM point_bangunan_fix WHERE kecamatan = :slctAdmin AND jenis_bang = :slctBangunan ORDER BY id_bang ASC");

        $params = ["slctAdmin"=>$slctAdmin,"slctBangunan"=>$slctBangunan];
        $sql->execute($params);
